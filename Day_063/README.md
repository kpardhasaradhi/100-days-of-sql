...
## Approach  
**Step 1:** Create a CTE to get all subscription records before January 1, 2021.  
**Step 2:** Use the `ROW_NUMBER()` function to rank each record by event date (latest first) for every customer.  
**Step 3:** Select only the record with `rno = 1`, which represents the most recent subscription before 2021.  
**Step 4:** Use the `DATE_ADD()` function to add the subscription period (in months) to the event date to calculate the `valid_until` date.  
**Step 5:** Filter only those customers whose `valid_until` date is on or after January 1, 2021, and order the results by `customer_id`.  
